...
node scripts/publish-runtime-toggle-smoke.mjs
```
...
For daily checks, `stable-validation` now runs build in a fast mode (`FLUXAURA_FAST_VALIDATE=1`) to reduce CPU/RAM overhead from compressed-size reporting. Use full debug build output only when diagnosing failures:

```powershell
$env:FLUXAURA_BUILD_DEBUG='1'
node scripts/stable-validation.mjs
```
